...
local_chrome_version(chromote::chrome_versions_list()[[2,1]])#"113.0.5672.0")
...
scrape_owner = function(owner_name,
  ...
  session <- selenider_session(session = "selenium",
                             browser = 'chrome',
                  options = selenium_options(selenium_client_options(capabilities = 
                                                                       selenium::chrome_options(args=c('--headless',
                                                                            "--disable-gpu")),
                                                                                timeout = 60))
                             )
  ...
  elem_click(search_button)
  elem_click(details_button)

  
  business_name <- s('#businessEntityName') %>% elem_text()
    
  business_details <- s('#trDetailsBody') %>% elem_text()
  ...
  elem_click(officer_button[[2]])
  
  officer <- tryCatch({s('#officeTable') %>% find_element('tbody')},
                      error = function(cond){
                              cond
                              }
                      ) 
  if('error' %in% class(officer)){
    ...
    if(depth>0 ){
      ...
      results =  foreach(index = 1:nrow(owner_table),
                       .combine = 'rbind') %do% {
  ...
  #found results
    ...
    if(sum(finance_inds)>0){
                        ...
                        .combine = 'rbind') %do% {
                          fin_owner_scrape = tryCatch({
                           ...
                           if('error' %in% class(fin_owner_scrape)){
                             ...
                             return(foreach(index = 1:nrow(owner_fin),
                                            .combine = 'rbind') %do% {
                                              cbind(owner_fin[index,],
                                                    business_details_table_parse)
                                            }
                                    )
                           }
                          return(fin_owner_scrape)
      ...
      if(sum(!finance_inds)>0){
        ...
        owners_non_fin =foreach(index = 1:nrow(owners_non_fin),
                       .combine = 'rbind') %do% {
                         cbind(owners_non_fin[index,],
                               business_details_table_parse)
                         }
...
plan(multisession)
print(Sys.time())
server <- selenium_server(path = getwd())
wait_for_selenium_available(error = TRUE)
owner_data <- foreach(index =1:10,#nrow(target_properties),
                      .combine = 'rbind',
                      .options.RNG = 8989,
                      .export = financial_marker_string) %dorng% {
                        ...
                        if('error' %in% class(property_owner_info)){
                          property_owner_info <- c(rep(NA, 13),
                                   situs_pID,
                                   situs_address,
                                   owner_name,
